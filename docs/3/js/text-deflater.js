...
//    static valid(base64) {atob(base64)}
    ...
    //static toBase64UrlFromText(text){return this.toBase64Url(this.fromU8a(this.fromText(text)))}
}
...
//    static fromU8a(bytes) {return Base64.toBase64Url(Base64.fromU8a(bytes))}
    static toU8a(base64url) {return Base64.toU8a(this.#toBase64(base64url));}
    /*
    static toU8a(base64url) {
        [...new Array(3)].map((_,i)=>{
            const padding = '='.repeat(i);
            const base64 = base64url.replace(/-/g, '+').replace(/_/g, '/') + padding;
            console.log(base64)
            try {return Base64.toU8a(base64)}
            catch(e){}
        })
    ...
    */
    ...
    static toBase64(base64url) {
        const base64 = base64url.replace(/-/g, '+').replace(/_/g, '/');
//        const B = base64+('='.repeat(base64.length % 4));
        let B = base64;
        while (B.length % 4) {B += '=';}
        console.log(B)
        return B;
